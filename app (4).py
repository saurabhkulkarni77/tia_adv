...
with st.sidebar:
    ...
    try:
        ...
        st.markdown('<span style="color:#00e676;font-size:13px;font-family:JetBrains Mono,monospace">● API key loaded from secrets</span>', unsafe_allow_html=True)
    except Exception:
        _api_key = st.text_input("Gemini API Key", type="password", placeholder="AIza…")
        if _api_key:
            st.markdown('<span style="color:#00e676;font-size:13px;font-family:JetBrains Mono,monospace">● API Connected</span>', unsafe_allow_html=True)
        else:
            st.markdown('<span style="color:#ff3b3b;font-size:13px;font-family:JetBrains Mono,monospace">● No API Key</span>', unsafe_allow_html=True)
